    ...
    function renderCartItems(orderItems) {
        ...
        orderItems.forEach((orderItem) => {
            ...
            cardEdit.addEventListener("click", () => {
                ...
                document.getElementById("Additional-requests-text").value = orderItem.additionalRequests;
                ...
                saveButton.onclick = () => {
                    const updatedRequest = document.getElementById("Additional-requests-text").value;
                    ...
                    .then(updatedOrder => {
                        ...
                        const cardDesc = document.querySelector(`.triggerDiv[data-id="${currentOrderItemId}"] .food-desc`);
                        if (cardDesc) {
                            cardDesc.innerText = updatedRequest;  // Update the food description text
                        }
                    })
                    .catch(error => console.error("Error updating additional request:", error));
                };
            });
    ...
    if (counter > 1) {
        ...
        .then(response => {
            if (response.ok) {
                ...
                cardPos.remove(); // Remove the item from UI
...
function getCurrentOrderItems() {
    ...
    return orderItems;
            ...
            cardAddBtn.addEventListener("click", () => {
                let counter = parseInt(cardQtyNumber.textContent);
                counter++;
                cardQtyNumber.textContent = counter;
            });
        });
    }
